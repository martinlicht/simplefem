...
inline SparseMatrix FEECBrokenElevationMatrix( Mesh& mesh, int n, int k, int r )
{
    ...
    const std::vector<MultiIndex> multis_high = generateMultiIndices( IndexRange( 0, n ), r+1 );
    ...
    assert( multis_high.size() == binomial( n+r+1, n ) );
    ...
    for( int p = 0; p <= n; p++ )
    {
        ...
        MultiIndex high_poly = low_poly + p;
