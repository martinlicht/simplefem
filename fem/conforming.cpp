...
#include "global.inclsullivan.hpp"
#include "global.inclafw.hpp"
#include "global.inclwhitney.hpp"
#include "global.flags.hpp"
#include "global.avgsullivan.hpp"
    ...
    for( int s  = 0; s   < num_volumes;                s++ )       // go over all the volumes
    {
        ...
        const MultiIndex alpha_vol = MultiIndex( IndexRange(0,n), [&alpha,&inclusion]( int p ) -> int {
                                            ...
                                            if( inclusion.has_value_in_range(p) )
                                                return alpha.at( inclusion.get_preimage_of(p) );
                                            else
                                                return 0;
                                        } );
    ...
    for( int s  = 0; s   < num_volumes;                s++  )    // go over all the volumes
    {
        ...
        for( int j = 0; j <= k; j++ )
        {
            ...
            const MultiIndex alpha_vol = MultiIndex( IndexRange(0,n), [&alpha_effective,&inclusion]( int p ) -> int {
                                                assert( inclusion.getTargetRange().contains(p) );
                                                if( inclusion.has_value_in_range(p) )
                                                    return alpha_effective.at( inclusion.get_preimage_of(p) );
                                                else
                                                    return 0;
                                            } );
