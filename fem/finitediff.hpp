#ifndef INCLUDEGUARD_FEM_FINITEDIFF
        ...
        AlternatingForm exteriorderivative( Float stepsize = 0.00001 ) const 
        {
            ...
            for( int p = 0; p < d; p++ ) {
                ...
                if( not src_form.rangecontains(p) ) {
                    ...
                    pattern.at(p).at( new_form_index, src_form_index ) = signpower( new_form.rangeposition(p) );
...
//                 std::cout << dim_src << space << dim_dst << space << pattern.size() << std::endl;
