...
int main( int argc, char *argv[] )
{
            ...
            for( int i = 0; i < number_of_samples; i++ ){
                ...
                for( int e = 0; e < M.count_simplices(M.getinnerdimension()-1); e++ )
                {
                    ...
                    if( M.get_flag( n-1, e ) == SimplexFlag::SimplexFlagDirichlet ) {
                        ...
                        // LOG << "Non-Dirichlet face " << e << " with values:" << tab;
                        // for( int j = e * dim; j < (e+1)*dim; j++ )
    ...
    for( int k = 0; k <= n; k++ ) 
    {
        ...
        for( int r = r_min; r <= r_max; r++ ) 
        {
            Assert( errors[k][l-l_min][r-r_min] < desired_closeness_for_sqrt, errors[k][l-l_min][r-r_min], desired_closeness_for_sqrt );
        }
