...
int main( int argc, char *argv[] )
{
    ...
    for( int r      = r_min; r      <=      r_max; r++      ) 
    {
        ...
        for( int i = 0; i < experiments_volume_field.size(); i++ )
            Assert( errors_volume[i][l-l_min][r-r_min] < desired_closeness_for_sqrt, errors_volume[i][l-l_min][r-r_min], desired_closeness_for_sqrt, errors_volume[i][l-l_min][r-r_min]/desired_closeness );
    }
