        ...
        if(true){
            ...
            experiments_sol.push_back( 
                [xfeq](const FloatVector& vec) -> FloatVector{
                    assert( vec.getdimension() == 1 );
                    return FloatVector({ std::sin( xfeq * Constants::twopi * vec[0] ) });
                }
            );
            ...
            experiments_grad.push_back( 
                [xfeq](const FloatVector& vec) -> FloatVector{
                    assert( vec.getdimension() == 1 );
                    return FloatVector( { 
                            xfeq * Constants::twopi * std::cos( xfeq * Constants::twopi * vec[0] )
                        });
                }
            );
            ...
            experiments_rhs.push_back( 
                [xfeq](const FloatVector& vec) -> FloatVector{
                    assert( vec.getdimension() == 1 );
                    return FloatVector({ 
                        xfeq*xfeq * Constants::fourpisquare * std::sin( xfeq * Constants::twopi * vec[0] )
                     });
                }
            );
